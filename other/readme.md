...
#### 2、手撕射线与球判断是否相交
#### 3、好多从0写起来的c++，比如有个树的问题或者就算是反转链表（难道我真的是笨蛋
#### 4、快排（不应该啊不应该。先指定第一个数是key，然后最右的右卫兵向左移动找比key小的到停下，最左的卫兵找比key大的到停下，交换，往复到两个卫兵相遇，然后交换卫兵所在的数和key-【结束递归的条件是left <= right】
...
#### 2.5 early-z
 在传统的渲染管线中，片元着色器先运行，生成片元的颜色，然后再与深度缓存中的 Z 值进行比较，决定该片元是否被保留。如果该片元被遮挡（即它的 Z 值大于已有片元），它的颜色计算就会被丢弃。
#### 3、三角形质心公式
...
二更，感觉就是算uv的时候不能用经过近大远小矫正过的去做插值
解决：
...
#### 14 间接光照、直接光照
#### 15 UE的新技术，一直没听清名字，华为一二面都问了(大胆猜测是lumen)
#### 16 两个向量之间是否在同一个平面上
#### 17 什么时候会用到正交而不是透视【ui渲染的时候】
#### 18 如何优化shader
...
#### 8、死锁的必要条件以及进程之间的通信
#### 9、map的底层实现
是红黑树不是平衡二叉树，救命  而且set也是红黑树
二叉查找树、平衡树（AVL）（解决前者退化为链表）、红黑树（相比前者，在插入删除，不会频繁频繁调整）
...
#### 21、完全二叉树和红黑树，谁的查询插入删除比较快
全是红黑树，那么前者好在哪里了：
如果你需要的是高效的查找、插入和删除，红黑树是更好的选择；但如果你需要的是堆的功能或空间上的简化（完全二叉树是可以直接转换成数组的），完全二叉树更适合。  
完全二叉树是一棵除了最后一层之外，其它每一层都是完全填满的二叉树，最后一层的节点按从左到右的顺序排列，没有缺失。
所以完全二叉树的好处在于简单以及更好实现堆（堆只需要知道最大的or最小的，用不到红黑树）
#### 22、模板（原来之前全部记成重载了，其实是那个T，比如说我要实现那个栈，我会用T当放到栈里的变量）
...
[lua与c#交互](https://github.com/derkder/plus-ultra/blob/main/BIG/lua/lua%E4%B8%8Ec%23%E4%BA%A4%E4%BA%92.md)   
